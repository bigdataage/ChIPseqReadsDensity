...
#####################################################################################################################################################################
## example:       python  AllGenes_HTSeq_middle.py      -inputFile  5-unique/day0_rep1.bam       -fragSize  200      -outputFile  7-HTSeq/1-ReadsDensity/middle/day0_rep1
...
for oneRead in bamFile: 
    if  (oneRead.aligned) and (oneRead.iv.start > fragSize) :
        ...
        ##print("###########################################")
        ##print(oneRead.iv)       
        if oneRead.iv.strand == "+":
            oneRead.iv.start = oneRead.iv.start +  fragSize/2
            oneRead.iv.end = oneRead.iv.start +  1
            positiveNum += 1
        if oneRead.iv.strand == "-":
            oneRead.iv.end = oneRead.iv.end -  fragSize/2 + 1
            oneRead.iv.start = oneRead.iv.end - 1
            negativeNum += 1
        ##print(oneRead.iv)
...
fOut0.write( "positiveNum" + ":\t" + str(positiveNum) + "\n" )
fOut0.write( "negativeNum" + ":\t" + str(negativeNum) + "\n" )
fOut0.close()
