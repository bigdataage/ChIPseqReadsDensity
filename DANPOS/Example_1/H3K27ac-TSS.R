...
AllResults_g <- "AllResults_H3K27ac"
...
Eight_NOL <- function(AAVEED_1,  AAVEED_2,   AAVGFP_1, AAVGFP_2,  ctrl_1, ctrl_2,   KO_1,   KO_2,  File_Local, Title_Local, Ymin_Local=0, Ymax_Local=12) {  
  BinNum_Local = 500
  ...
  x_Axis_Local     <- c( Position_Local,  Position_Local,   Position_Local,    Position_Local,   Position_Local,  Position_Local,   Position_Local,   Position_Local)                       
  y_Axis_Local     <- c( AAVEED_1_Y,  AAVEED_2_Y,   AAVGFP_1_Y,    AAVGFP_2_Y, ctrl_1_Y, ctrl_2_Y, KO_1_Y, KO_2_Y ) 
  SampleType_Local <- c(  rep("AAV_EED_1", BinNum_Local),  rep("AAV_EED_2", BinNum_Local), rep("AAV_GFP_1", BinNum_Local),  rep("AAV_GFP_2", BinNum_Local),  rep("EED_Ctrl_1", BinNum_Local), rep("EED_Ctrl_2", BinNum_Local),    rep("EED_KO_1", BinNum_Local),  rep("EED_KO_2", BinNum_Local)  )  
  ...
  FigureTemp <- ggplot(DataFrame_Local, aes(x=xAxis, y=yAxis,  color=SampleType) )  +   
    xlab("Relative Distance (kb)") +  ylab("H3K27ac Signal") +  ggtitle(Title_Local) + 
    scale_colour_manual( values=c("purple",  "purple4",    "red",  "red4",   "green",  "green4",  "blue", "blue4") ) +    
    geom_line(size=0.5) + 
...
Four_NOL <- function(AAVEED_1, AAVEED_2,   AAVGFP_1, AAVGFP_2,    ctrl_1, ctrl_2,    KO_1,   KO_2,   File_Local, Title_Local, Ymin_Local=0, Ymax_Local=12) {  
  BinNum_Local = 500
  Position_Local      <-  seq(from = -5,  by=0.02,  length.out=BinNum_Local)
  AAVEED <- (AAVEED_1 + AAVEED_2)/2
  AAVGFP <- (AAVGFP_1 + AAVGFP_2)/2
  ctrl   <- (ctrl_1 + ctrl_2)/2
  KO     <- (KO_1 + KO_2)/2
...
#################################################################### Start ##########################################################################################################################################
#################################################################### Start ##########################################################################################################################################
AAVEED_1    <- read.table("H3K27ac/Down/H3K27ac_AAVEED1_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")  
AAVEED_2    <- read.table("H3K27ac/Down/H3K27ac_AAVEED2_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")  
AAVGFP_1    <- read.table("H3K27ac/Down/H3K27ac_AAVGFP1_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")
AAVGFP_2    <- read.table("H3K27ac/Down/H3K27ac_AAVGFP2_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")
ctrl_1      <- read.table("H3K27ac/Down/H3K27ac_ctrl1_center_heatmap/a.xls",      header=TRUE,   sep="",   quote = "",   comment.char = "")
ctrl_2      <- read.table("H3K27ac/Down/H3K27ac_ctrl2_center_heatmap/a.xls",      header=TRUE,   sep="",   quote = "",   comment.char = "")
KO_1        <- read.table("H3K27ac/Down/H3K27ac_KO1_center_heatmap/a.xls",        header=TRUE,   sep="",   quote = "",   comment.char = "")
KO_2        <- read.table("H3K27ac/Down/H3K27ac_KO2_center_heatmap/a.xls",        header=TRUE,   sep="",   quote = "",   comment.char = "")
...
Eight_NOL( AAVEED_1,  AAVEED_2,  AAVGFP_1,  AAVGFP_2, ctrl_1,  ctrl_2,  KO_1, KO_2 , paste(AllResults_g, "/A-H3K27ac-Down.svg", sep=""),  "KO Down (95 Genes)",   Ymin_Local=0,  Ymax_Local=7)   

Four_NOL( AAVEED_1,  AAVEED_2,  AAVGFP_1,  AAVGFP_2, ctrl_1,  ctrl_2,  KO_1, KO_2 , paste(AllResults_g, "/A-H3K27ac-Down-Average.svg", sep=""),  "KO Down (95 Genes)",   Ymin_Local=0,  Ymax_Local=7)                     
...
#################################################################### Start ##########################################################################################################################################
#################################################################### Start ##########################################################################################################################################
AAVEED_1    <- read.table("H3K27ac/Nochange/H3K27ac_AAVEED1_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")  
AAVEED_2    <- read.table("H3K27ac/Nochange/H3K27ac_AAVEED2_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")  
AAVGFP_1    <- read.table("H3K27ac/Nochange/H3K27ac_AAVGFP1_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")
AAVGFP_2    <- read.table("H3K27ac/Nochange/H3K27ac_AAVGFP2_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")
ctrl_1      <- read.table("H3K27ac/Nochange/H3K27ac_ctrl1_center_heatmap/a.xls",      header=TRUE,   sep="",   quote = "",   comment.char = "")
ctrl_2      <- read.table("H3K27ac/Nochange/H3K27ac_ctrl2_center_heatmap/a.xls",      header=TRUE,   sep="",   quote = "",   comment.char = "")
KO_1        <- read.table("H3K27ac/Nochange/H3K27ac_KO1_center_heatmap/a.xls",        header=TRUE,   sep="",   quote = "",   comment.char = "")
KO_2        <- read.table("H3K27ac/Nochange/H3K27ac_KO2_center_heatmap/a.xls",        header=TRUE,   sep="",   quote = "",   comment.char = "")
...
Eight_NOL( AAVEED_1,  AAVEED_2,  AAVGFP_1,  AAVGFP_2, ctrl_1,  ctrl_2,  KO_1, KO_2 , paste(AllResults_g, "/B-H3K27ac-Retain.svg", sep=""),  "KO Retain (5681 Genes)",   Ymin_Local=0,  Ymax_Local=7)   

Four_NOL( AAVEED_1,  AAVEED_2,  AAVGFP_1,  AAVGFP_2, ctrl_1,  ctrl_2, KO_1, KO_2 , paste(AllResults_g, "/B-H3K27ac-Retain-Average.svg", sep=""),  "KO Retain (5681 Genes)",   Ymin_Local=0,  Ymax_Local=7)                     
...
#################################################################### Start ##########################################################################################################################################
#################################################################### Start ##########################################################################################################################################
AAVEED_1    <- read.table("H3K27ac/Up/H3K27ac_AAVEED1_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")  
AAVEED_2    <- read.table("H3K27ac/Up/H3K27ac_AAVEED2_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")  
AAVGFP_1    <- read.table("H3K27ac/Up/H3K27ac_AAVGFP1_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")
AAVGFP_2    <- read.table("H3K27ac/Up/H3K27ac_AAVGFP2_center_heatmap/a.xls",    header=TRUE,   sep="",   quote = "",   comment.char = "")
ctrl_1      <- read.table("H3K27ac/Up/H3K27ac_ctrl1_center_heatmap/a.xls",      header=TRUE,   sep="",   quote = "",   comment.char = "")
ctrl_2      <- read.table("H3K27ac/Up/H3K27ac_ctrl2_center_heatmap/a.xls",      header=TRUE,   sep="",   quote = "",   comment.char = "")
KO_1        <- read.table("H3K27ac/Up/H3K27ac_KO1_center_heatmap/a.xls",        header=TRUE,   sep="",   quote = "",   comment.char = "")
KO_2        <- read.table("H3K27ac/Up/H3K27ac_KO2_center_heatmap/a.xls",        header=TRUE,   sep="",   quote = "",   comment.char = "")
...
Eight_NOL( AAVEED_1,  AAVEED_2,  AAVGFP_1,  AAVGFP_2, ctrl_1,  ctrl_2,  KO_1, KO_2 , paste(AllResults_g, "/C-H3K27ac-Up.svg", sep=""),          "KO Up (283 Genes)",   Ymin_Local=0,  Ymax_Local=7)   

Four_NOL(  AAVEED_1,  AAVEED_2,  AAVGFP_1,  AAVGFP_2, ctrl_1,  ctrl_2,  KO_1, KO_2 , paste(AllResults_g, "/C-H3K27ac-Up-Average.svg", sep=""),  "KO Up (283 Genes)",   Ymin_Local=0,  Ymax_Local=7)                     
